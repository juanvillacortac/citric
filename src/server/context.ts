...
export class ServerContext {
  ...
  static async fromManifest(
    ...
    for (const [self, module] of Object.entries(manifest.routes)) {
      const url = new URL(self.replace("+", "_"), baseUrl).pathname;
      ...
      const isMiddleware = path.endsWith("/_middleware"); // ||
      const isLayout = path.endsWith("/_layout"); // ||
      const isPage = path.endsWith("/_page"); // ||
      // path.endsWith("/_middleware.ts") ||
      // path.endsWith("/_middleware.jsx") ||
      // path.endsWith("/_middleware.js");
      if (isPage && !isMiddleware) {
        ...
        const { default: component } = module as LayoutModule;
        layouts.push({
    ...
    for (const [self, module] of Object.entries(manifest.islands)) {
      ...
      const sanitizedName = sanitizeIslandName(baseRoute);
      const matches = islands.filter((i) => i.name.startsWith(sanitizedName));
  ...
  /**
   * Identify which layouts should be applied for a request,
   * chain them and return a single state
   */
  // #composeLayouts(layouts: LayoutRoute[]) {
  //   return (req: Request, connInfo: ConnInfo) => {
  //     // identify middlewares to apply, if any.
  //     // middlewares should be already sorted from deepest to shallow layer
  //     const lyts = selectLayouts(req.url, layouts);

  //     const handlers: (() =>
  //       | Record<string, unknown>
  //       | Promise<Record<string, unknown>>)[] = [];

  //     const ctx: LayoutHandlerContext = {
  //       parent() {
  //         const handler = handlers.shift()!;
  //         return Promise.resolve(handler());
  //       },
  ...
  //     for (const ly of lyts) {
  //       if (ly.handler) {
  //         const handler = ly.handler;
  //         handlers.push(() => handler(req, ctx));
  //       }
  //     }
  ...
  #handlers(): [
    ...
    const genRender = <Data = undefined>(
      ...
      return (
        ...
        return async (data?: Data) => {
          ...
          const layouts = selectLayouts(req.url, this.#layouts);
      ...
      if (typeof route.handler === "function") {
        routes[route.pattern] = (req, ctx, params) =>
          (route.handler as Handler)(req, {
            ...ctx,
            params,
            render: createRender(req, params, undefined, ctx.state),
    ...
    const unknownHandler: rutt.Handler<RouterState> = (req, ctx) =>
      this.#notFound.handler(req, {
        ...ctx,
        render: createUnknownRender(req, {}, undefined, ctx.state),
      });
    ...
    const errorHandler: rutt.ErrorHandler<RouterState> = (req, ctx, error) => {
      ...
      return this.#error.handler(req, {
        ...ctx,
        error,
        render: errorHandlerRender(req, {}, error, ctx.state),
      });
    };
 ...
 * Return a list of middlewares that needs to be applied for request url
 * @param url the request url
 * @param layouts Array of middlewares handlers and their routes as path-to-regexp style
 */
export function selectLayouts(url: string, layouts: LayoutRoute[]) {
  const selectedLayouts: Layout[] = [];
  const reqURL = new URL(url);

  for (const { compiledPattern, component } of layouts) {
    const res = compiledPattern.exec(reqURL);
    if (res) {
      selectedLayouts.push({ component });
    }
  }
...
function sanitizeIslandName(name: string): string {
  ...
  return toPascalCase(
    name.substring(0, name.lastIndexOf(".island")).split("/").at(-1)!
  );
}
