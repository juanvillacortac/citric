...
export type StartOptions = ServeInit &
  FreshOptions & {
     ...
     *
     * This option is experimental and may be removed in a future Lemonade release.
     */
...
export interface RouteConfig {
   ...
   * that are generated by lemonade. Additional scripts and styles can be added
...
export interface LayoutRoute extends Layout {
  /**
   * path-to-regexp style url path
   */
  pattern: string;
...
export interface LayoutProps<State = Record<string, unknown>> {
  state: State;
  Component: ComponentType<Record<never, never>>;
}

export interface LayoutModule<State = Record<string, unknown>> {
  default: ComponentType<LayoutProps<State>>;
}

export interface Layout<State = Record<string, unknown>> {
  component: ComponentType<LayoutProps<State>>;
}
