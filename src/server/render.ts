...
export async function render<Data, State>(
  ...
  const renderTree = (layouts: Layout[], children: VNode<any>): VNode<any> => {
    const leaf = layouts.shift();
    if (leaf) {
      return h(leaf.component, {
        state: opts.state ?? {},
        Component() {
          return renderTree(layouts, children);
        },
      });
    }
    return children;
    ...
    children: h(HEAD_CONTEXT.Provider, {
      ...
      children: h(opts.app.default, {
        Component() {
          return renderTree(
            opts.layouts,
            h(opts.route.component! as ComponentType<unknown>, props)
          );
        },
      }),
    }),
  ...
  if (ENCOUNTERED_ISLANDS.size > 0) {
    ...
    for (const island of ENCOUNTERED_ISLANDS) {
      ...
      imports.push([url, randomNonce] as const);
      script += `import $${idx} from "${url}";`;
      islandRegistry += `${island.id}:$${idx},`;
      idx++;
    }
